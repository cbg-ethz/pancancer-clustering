...
for (s in 1:EMn) {
  ...
  while (diffy>err) { 
    ...
    for (k in 1:kclust) {
      ...
      scorepar<-scoreparameters("bde",Datafull,
    ...
    for (i in 1:nit.internal) { 
      ...
      tauvec<-coltots/sum(coltots) 
      for (k in 1:kclust) {
        scorepar<-scoreparameters("bde",as.data.frame(Datafull),
    ...
    for (k in 1:3) {
      centers[[k]][cnt,]<-compareDAGs(m2graph(clustercenters[[k]]),
                                      BNs[[res$relabel[k]]]$DAG)[c("SHD","TP","FP")]
    }
    cnt<-cnt+1
  }
...
In our example `r toString(round(perccorr))`% of observations were assigned to correct clusters for the EM run with highest likelihood which corresponds to seed `r toString(bestseed)`. Other EM runs have similar or identical accuracy. 
